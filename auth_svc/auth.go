...
func NewAuthServiceClient(config *util.Config) (*AuthServiceClient, error) {
	...
	conn, err := grpc.NewClient(fmt.Sprintf("%s:%d", config.AuthService.ServerAddress, config.AuthService.GRPCServerPort), grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
