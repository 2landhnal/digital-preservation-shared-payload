...
func NewCoreServiceClient(config *util.Config) (*CoreServiceClient, error) {
	...
	conn, err := grpc.NewClient(fmt.Sprintf("%s:%d", config.CoreService.ServerAddress, config.CoreService.GRPCServerPort), grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
